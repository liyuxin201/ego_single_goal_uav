...
namespace AutoFlight{
	...
	protected:
		...
		ros::Subscriber externalPoseTargetSub_;
		ros::Subscriber externalOffboardSetpointSub_;
		...
		// parameter
		double takeoffHgt_ ;
		bool usePx4Offboard_ = false;
		bool releaseControlOnGoal_ = false;
		bool enableControlOutput_ = true;
		bool listenClickGoal_ = true;
		bool enforceClickGoalRange_ = false;
		double clickGoalRangeX_ = 10.0;
		double clickGoalRangeY_ = 10.0;
		std::string goalTopic_ = "/move_base_simple/goal";
		std::string externalPoseTargetTopic_;
		std::string externalOffboardSetpointTopic_;
		double externalOffboardTimeout_ = 0.0;
		...
		bool relinquishedControl_ = false;
		bool externalOffboardSetpointReceived_ = false;
		bool externalOffboardControlActive_ = false;
		ros::Time lastExternalOffboardSetpointTime_;
	...
	public:
		...
		void externalPoseTargetCB(const geometry_msgs::PoseStamped::ConstPtr& ps);
		void externalOffboardSetpointCB(const mavros_msgs::PositionTarget::ConstPtr& setpoint);
	...
	static void updateKinematicState(const Eigen::Vector3d& currVel,
									 const ros::Time& currTime,
									 bool& stateUpdateFirstTime,
									 Eigen::Vector3d& prevVel,
									 Eigen::Vector3d& currAcc,
									 ros::Time& prevStateTime);
	static bool isExternalOffboardControlActive(bool externalOffboardSetpointReceived,
												 const ros::Time& lastExternalOffboardSetpointTime,
												 const ros::Time& now,
												 double timeoutSec);
	static bool isPointInLocalXYRange(double pointX,
									   double pointY,
									   double centerX,
									   double centerY,
									   double rangeX,
