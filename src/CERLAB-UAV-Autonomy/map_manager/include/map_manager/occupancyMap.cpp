...
namespace mapManager{
	...
	void occMap::initParam(){
		...
		if (not this->nh_.getParam(this->ns_ + "/point_cloud_world_frame", this->pointcloudWorldFrame_)){
			...
			cout << this->hint_ << ": Pointcloud world-frame mode: " << this->pointcloudWorldFrame_ << endl;
		}
	...
	void occMap::publish2DOccupancyGrid(){
		...
		mapMsg.data.resize(width * height, 0);

		double z = 0.5;
		Eigen::Vector3d zPos (0.0, 0.0, z);
		Eigen::Vector3i zIdxVec;
		this->posToIndex(zPos, zIdxVec);
		this->boundIndex(zIdxVec);
		int zIdx = zIdxVec(2);
		for (int x=minRangeIdx(0); x<=maxRangeIdx(0); ++x){
			for (int y=minRangeIdx(1); y<=maxRangeIdx(1); ++y){
