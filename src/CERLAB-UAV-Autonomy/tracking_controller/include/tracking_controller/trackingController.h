...
namespace controller{
	class trackingController{
		private:
			...
			double hoverThrust_;
			bool verbose_;
			bool passthroughRawSetpoint_ = false;
			std::string rawSetpointMode_ = "full_state";
			Eigen::Vector3d velocityHeightPositionGain_ = Eigen::Vector3d(0.8, 0.8, 1.0);
			Eigen::Vector3d velocityHeightMaxVelocity_ = Eigen::Vector3d(3.0, 3.0, 1.0);
		...
		public:
			...
			void publishRawSetpointFromTarget();
			void publishVelocityHeightSetpointFromTarget();
			static mavros_msgs::PositionTarget buildRawSetpointFromTarget(const tracking_controller::Target& target,
																		   const ros::Time& stamp);
			static mavros_msgs::PositionTarget buildVelocityHeightSetpointFromTarget(
