...
namespace controller{
	namespace{
		double clampAbs(double value, double maxAbs){
			const double limit = std::abs(maxAbs);
			if (limit <= 0.0){
				return 0.0;
			}
		...
		bool loadVector3Param(const ros::NodeHandle& nh, const std::string& name, Eigen::Vector3d& value){
			std::vector<double> temp;
			if (!nh.getParam(name, temp) || temp.size() < 3){
				return false;
			}
			value(0) = temp[0];
			value(1) = temp[1];
	...
	void trackingController::initParam(){
		...
		if (not this->nh_.getParam("controller/passthrough_raw_setpoint", this->passthroughRawSetpoint_)){
			this->passthroughRawSetpoint_ = false;
		}
		cout << "[trackingController]: Raw setpoint passthrough is set to: " << this->passthroughRawSetpoint_ << endl;

		if (not this->nh_.getParam("controller/raw_setpoint_mode", this->rawSetpointMode_)){
			this->rawSetpointMode_ = "full_state";
		}
		cout << "[trackingController]: Raw setpoint mode is set to: " << this->rawSetpointMode_ << endl;

		if (!loadVector3Param(this->nh_, "controller/velocity_height_position_p", this->velocityHeightPositionGain_)){
			this->velocityHeightPositionGain_ = Eigen::Vector3d(0.8, 0.8, 1.0);
		}
		cout << "[trackingController]: Velocity-height position gain is set to: ["
			 << this->velocityHeightPositionGain_(0) << ", "
			 << this->velocityHeightPositionGain_(1) << ", "
			 << this->velocityHeightPositionGain_(2) << "]." << endl;

		if (!loadVector3Param(this->nh_, "controller/velocity_height_max_velocity", this->velocityHeightMaxVelocity_)){
			this->velocityHeightMaxVelocity_ = Eigen::Vector3d(3.0, 3.0, 1.0);
		}
		cout << "[trackingController]: Velocity-height max velocity is set to: ["
			 << this->velocityHeightMaxVelocity_(0) << ", "
			 << this->velocityHeightMaxVelocity_(1) << ", "
			 << this->velocityHeightMaxVelocity_(2) << "]." << endl;
	}
	...
	void trackingController::cmdCB(const ros::TimerEvent&){
		...
		if (this->passthroughRawSetpoint_){
			if (this->rawSetpointMode_ == "velocity_height"){
				this->publishVelocityHeightSetpointFromTarget();
			}
			else{
				this->publishRawSetpointFromTarget();
			}
	...
	void trackingController::publishRawSetpointFromTarget(){
		this->accCmdPub_.publish(trackingController::buildRawSetpointFromTarget(this->target_, ros::Time::now()));
	}

	void trackingController::publishVelocityHeightSetpointFromTarget(){
		this->accCmdPub_.publish(trackingController::buildVelocityHeightSetpointFromTarget(
			this->target_,
			this->odom_.pose.pose,
			this->velocityHeightPositionGain_,
			this->velocityHeightMaxVelocity_,
			ros::Time::now()));
	}

	mavros_msgs::PositionTarget trackingController::buildRawSetpointFromTarget(
		const tracking_controller::Target& target,
		const ros::Time& stamp){
		mavros_msgs::PositionTarget cmdMsg;
		cmdMsg.header.stamp = stamp;
		cmdMsg.header.frame_id = "map";
		cmdMsg.coordinate_frame = mavros_msgs::PositionTarget::FRAME_LOCAL_NED;
		cmdMsg.type_mask = mavros_msgs::PositionTarget::IGNORE_YAW_RATE;
		cmdMsg.position.x = target.position.x;
		cmdMsg.position.y = target.position.y;
		cmdMsg.position.z = target.position.z;
		cmdMsg.velocity.x = target.velocity.x;
		cmdMsg.velocity.y = target.velocity.y;
		cmdMsg.velocity.z = target.velocity.z;
		cmdMsg.acceleration_or_force.x = target.acceleration.x;
		cmdMsg.acceleration_or_force.y = target.acceleration.y;
		cmdMsg.acceleration_or_force.z = target.acceleration.z;
		cmdMsg.yaw = target.yaw;
		cmdMsg.yaw_rate = 0.0;
		return cmdMsg;
	}

	mavros_msgs::PositionTarget trackingController::buildVelocityHeightSetpointFromTarget(
		const tracking_controller::Target& target,
		const geometry_msgs::Pose& currentPose,
		const Eigen::Vector3d& positionGain,
		const Eigen::Vector3d& maxVelocity,
		const ros::Time& stamp){
		mavros_msgs::PositionTarget cmdMsg;
		cmdMsg.header.stamp = stamp;
		cmdMsg.header.frame_id = "map";
		cmdMsg.coordinate_frame = mavros_msgs::PositionTarget::FRAME_LOCAL_NED;
		cmdMsg.type_mask = mavros_msgs::PositionTarget::IGNORE_AFX |
		...
		const double vx = target.velocity.x + positionGain(0) * (target.position.x - currentPose.position.x);
		const double vy = target.velocity.y + positionGain(1) * (target.position.y - currentPose.position.y);

		cmdMsg.position.z = target.position.z;
		cmdMsg.velocity.x = clampAbs(vx, maxVelocity(0));
		cmdMsg.velocity.y = clampAbs(vy, maxVelocity(1));
		cmdMsg.velocity.z = 0.0;
		cmdMsg.yaw = target.yaw;
		cmdMsg.yaw_rate = 0.0;
		return cmdMsg;
	}
